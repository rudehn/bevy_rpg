...
//!
//! Pure utilities with no game-specific content: a parametric `Grid<T>`,
//! cellular-automata iteration, flood-fill region analysis, and a
//! best-of-N blob generator. All functions are deterministic given their
//! RNG state — `randomize_grid` uses `rand::rng()` (thread-local) so
//! tests that care about exact output should seed externally.
    ...
    T: Copy + Clone + PartialEq + Default,
{
    ...
    for dy in -radius..=radius {
        for dx in -radius..=radius {
            ...
            let nx = x + dx;
            let ny = y + dy;
            if grid.in_bounds(nx, ny) && *grid.at(nx, ny).unwrap() == floor_val {
                count += 1;
            }
        }
    }
...
pub fn cellular_automata_iteration<T>(
    grid: &mut Grid<T>,
    birth_threshold: i32,
    ...
    T: Copy + Clone + PartialEq + Default,
{
    ...
    if !grid.in_bounds(start_x, start_y)
        || *grid.at(start_x, start_y).unwrap_or(&wall_val) == wall_val
    {
...
pub fn size_score(width: i32, height: i32, min_w: i32, min_h: i32, max_w: i32, max_h: i32) -> f32 {
    ...
    let overshoot_penalty = if width > max_w || height > max_h {
        0.5
    } else {
        1.0
    };
